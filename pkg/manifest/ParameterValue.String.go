...
// String - return a string representation of the ParameterDataObject
func (pdo *ParameterDataObject) String() string {
	...
	case PdoParameterArray:
		return pdo.State.(PdoParameterArray).String()
	case PdoParameterEnum:
		return pdo.State.(PdoParameterEnum).String()
	case PdoParameterObject:
		return pdo.State.(PdoParameterObject).String()
	default:
		panic(fmt.Sprintf("unhandled default case in ParameterDataObject.String(): '%c'", pdo.State))
	}
}
